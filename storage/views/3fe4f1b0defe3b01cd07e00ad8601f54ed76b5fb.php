...
<body class="hold-transition skin-<?php echo Config::get('app.color_scheme', 'blue');; ?> sidebar-mini <?php echo e(Session::get('sidebarState')); ?>">
...
<div class="wrapper">
  ...
  <header class="main-header">
    ...
    <a href="<?php echo admin_url('dashboard'); ?>" class="logo">
      ...
      <span class="logo-lg"><?php echo __d('admin_lite', 'Control Panel'); ?></span>
    </a>
    ...
    <nav class="navbar navbar-static-top" role="navigation">
      ...
      <a href="#" id='sidebarToggle' class="sidebar-toggle" data-toggle="push-menu" role="button">
        <span class="sr-only"><?php echo __d('admin_lite', 'Toggle navigation'); ?></span>
      </a>
      <!-- Navbar Right Menu -->
        ...
        <ul class="nav navbar-nav" style="margin-right: 10px;">
            ...
            <li class="dropdown notifications-menu">
                ...
                <ul class="dropdown-menu">
                    <li class="header"><?php echo __d('admin_lite', 'Quick Create'); ?></li>
                    <li>
                        <ul class="menu">
                            <?php if(isset($menuItemsQuickCreate)): ?>
                                <?php foreach($menuItemsQuickCreate as $item): ?>
                                    <li><a href='<?php echo admin_url($item['uri']); ?>'><i class='fa fa-<?php echo $item['icon']; ?>'></i> <span> <?php echo $item['title']; ?></span></a></li>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </ul>
                    </li>
            ...
            <li class="dropdown language-menu">
                <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                  <i class='fa fa-language'></i> <?php echo $langName; ?>

                </a>
                <ul class="dropdown-menu">
                  <?php echo $langMenuLinks; ?>

                </ul>
            </li>

            <li><a href='<?php echo site_url(); ?>'><?php echo __d('admin_lite', 'View Site'); ?></a></li>
            ...
            <li class="dropdown user user-menu">
                ...
                <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                  ...
                  <img src="<?php echo resource_url($user->imagePath); ?>" class="user-image" alt="<?php echo __d('admin_lite', 'User Image'); ?>">
                ...
                <ul class="dropdown-menu">
                  <!-- The user image in the menu -->
                    <li class="user-header">
                        <img src="<?php echo resource_url($user->imagePath); ?>" class="img-circle" alt="<?php echo __d('admin_lite', 'User Image'); ?>">

                        <p>
                            <?php echo $user->realname;; ?> - <?php echo $user->role->name;; ?>

                            <small><?php echo __d('admin_lite', 'Member since {0}', $sinceDate);; ?></small>
                        </p>
                    </li>
                    ...
                    <li class="user-footer">
                        ...
                        <a href="<?php echo admin_url('users/profile'); ?>" class="btn btn-xs btn-default btn-flat"><i class="fa fa-user"></i> <?php echo __d('admin_lite', 'Profile'); ?></a>

                        <a href="<?php echo admin_url('users/'.$user->id.'/edit'); ?>" class="btn btn-xs btn-default btn-flat"><i class="fa fa-cog"></i> <?php echo __d('admin_lite', 'My Settings'); ?></a>

                        <a class="btn btn-xs btn-default btn-flat" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><i class="fa fa-lock"></i> <?php echo __d('admin_lite', 'Sign out'); ?></a>
                        <form id="logout-form" action="<?php echo admin_url('logout'); ?>" method="post"></form>
            ...
            <li id="notification_li">
                ...
                <div id="notificationContainer">
                    <div id="notificationTitle"><?php echo __d('admin_lite', 'Notifications'); ?></div>
                    <div id="notificationsBody" class="notifications"></div>
                    <div id="notificationFooter"><a href="<?php echo admin_url('notifications'); ?>"><?php echo __d('admin_lite', 'See All'); ?></a></div>
                </div>
            </li>
        </ul>
      </div>
  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
        ...
        <ul class="sidebar-menu" data-widget="tree">
            ...
            <li class="header"><?php echo __d('admin_lite', 'Menu'); ?></li>

            <?php if(isset($menuItems)): ?>
                <?php foreach($menuItems as $item): ?>
                    <li <?php echo ($baseUri == 'admin/'.$item['uri']) ? 'class="active"' : ''; ?>><a href='<?php echo admin_url($item['uri']); ?>'><i class='fa fa-<?php echo $item['icon']; ?>'></i> <span> <?php echo $item['title']; ?></span></a></li>
                <?php endforeach; ?>
            <?php endif; ?>

            <li class="header"><?php echo __d('admin_lite', 'Modules Administration'); ?></li>

            <?php if(isset($menuItemsModules)): ?>
            <?php if(count($menuItemsModules) == 0): ?>
                <li><a href='#'><span>No Items</span></a></li>
            <?php endif; ?>
                <?php foreach($menuItemsModules as $item): ?>
                    <li <?php echo ($baseUri == 'admin/'.$item['uri']) ? 'class="active"' : ''; ?>><a href='<?php echo admin_url($item['uri']); ?>'><i class='fa fa-<?php echo $item['icon']; ?>'></i> <span> <?php echo $item['title']; ?></span></a></li>
                <?php endforeach; ?>
            <?php endif; ?>

            <?php if($user->hasRole('administrator')): ?>
                <li class="header"><?php echo __d('admin_lite', 'System Administration'); ?></li>
                <?php if(isset($menuItemsSystem)): ?>
                    <?php foreach($menuItemsSystem as $item): ?>
                        <li <?php echo ($baseUri == 'admin/'.$item['uri']) ? 'class="active"' : ''; ?>><a href='<?php echo admin_url($item['uri']); ?>'><i class='fa fa-<?php echo $item['icon']; ?>'></i> <span> <?php echo $item['title']; ?></span></a></li>
                    <?php endforeach; ?>
                <?php endif; ?>
            <?php endif; ?>
  ...
  <footer class="main-footer">
    ...
    <!-- Default to the left -->
    <strong>Copyright &copy; <?php echo date('Y'); ?> <a href="http://www.novaframework.com/" target="_blank"><b>Nova Framework <?= $version; ?> / Kernel <?php echo VERSION; ?></b></a> - </strong> All rights reserved.
  </footer>
...
$(function () {
    <?php echo isset($jq) ? $jq : ''; ?>


    //notifications menu
    function loadlinks(){
        //load count
        $("#notificationcount").load("<?php echo admin_url('notifications/getnotificationscount'); ?>");
    }
    ...
    $("#notificationLink").click(function(){
        ...
        $("#notificationsBody").load("<?php echo admin_url('notifications/getnotifications'); ?>");

        //update database
        $.ajax({url: "<?php echo admin_url('notifications/removenotificationscount'); ?>"});
        //remove from view
    ...
    $('#sidebarToggle').on('click', function(e) {
        $.ajax({
            type: "POST",
            url: "<?php echo admin_url('dashboard/savestate'); ?>"
        });
    });
});

CKEDITOR.editorConfig = function( config ) {
    config.filebrowserBrowseUrl = '<?php echo admin_url('files/plain'); ?>';
    config.baseHref = '<?php echo site_url(); ?>';
