...
    <div class="box-body">
		...
		<form action='{{{ admin_url('sidebars/'.$sidebar->id) }}}' method='post'>
		...
		<div class='row'>
			...
			<div class='col-md-6'>
				...
				<div class="control-group">
				    ...
				    <input class="form-control" id='title' type="text" name="title" value="{{{ Input::old('title', $sidebar->title) }}}" />
				</div>

				<div class="control-group">
				    <label class="control-label" for='displayTitle'> Display Title</label>
				    <input id='displayTitle' type="checkbox" name="displayTitle" {{ (Input::old('displayTitle', $sidebar->displayTitle) == 'on' ? 'checked=checked' : '') }}" />
				</div>

				<div class="control-group">
				    <label class="control-label" for='class'> Class (optional classes comma seperated)</label>
				    <input class="form-control tags" id='class' type="text" name="class" value="{{{ Input::old('class', $sidebar->class) }}}" />
				</div>
			...
			<div class='col-md-6'>
				...
				<div class="control-group">
				    ...
				    @php
				    $options = ['Left', 'Right'];
				    $position = Input::old('position', $sidebar->position);
		...
		<div class="control-group">
		    ...
		    <textarea class="form-control ckeditor" id='content' name="content" rows='10'>{{ Input::old('content', $sidebar->content) }}</textarea>
		</div>
