    ...
    @if (CONFIG_STORE == 'database')
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                ...
                <div class="form-group">
                    <label class="col-sm-4 control-label" for="sitename">{{ __d('system', 'IP address access list') }}</label>
                    <div class="col-sm-8">
                        <table class='table table-striped table-hover table-bordered sorted_table'>
                        <tr>
                            <th colspan="2">{{ __d('system', 'Users set to office login only will only be able to login from the following IP addresses') }}</th>
                        </tr>
                        <tr>
                            <td colspan="2">{{ __d('system', 'You current IP address is') }} <?=Request::server('REMOTE_ADDR');?></td>
                        </tr>
                        <tr>
                            <th>{{ __d('system', 'IP') }}</th>
                        ...
                        <tbody id="ipextender">
                            ...
                            foreach($options['ipAccessList'] as $ip=>$name) {
                                ...
                                <td><input type='text' name='names[]' value='$name' />  <a href='#' class='ipremove'>{{ __d('system', 'Remove') }}</a></td>
                                </tr>";
                            $i++;}
                            @endphp
                        </tbody>
                        </table>
                        <p><a href="#" id="ipadd" class='btn btn-success'>{{ __d('system', 'Add Row') }}</a></p>
                    </div>
                </div>

                <div class="form-group">
                    <label class="col-sm-4 control-label" for="sitename">{{ __d('system', 'Developer Email Accounts') }}</label>
                    <div class="col-sm-8">
                        <table class='table table-striped table-hover table-bordered sorted_table'>
                        <tr>
                            <th colspan="2">{{ __d('system', 'Set email accounts for sending error reports to') }}.</th>
                        </tr>
                        <tr>
                            <th>{{ __d('system', 'Email') }}</th>
                            <th>{{ __d('system', 'Name') }}</th>
                        </tr>
                        <tbody id="devemailextender">
                            ...
                            foreach($options['devEmails'] as $email=>$name) {
                                ...
                                <td><input type='text' name='names[]' value='$name' />  <a href='#' class='devemailremove'>{{ __d('system', 'Remove') }}</a></td>
                                </tr>";
                            $i++;}
                            @endphp
                        </tbody>
                        </table>
                        <p><a href="#" id="devemailadd" class='btn btn-success'>{{ __d('system', 'Add Row') }}</a></p>
                    </div>
                </div>
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">{{ __d('system', 'Mail Settings') }}</h3>
            </div>
            <div class="box-body">
                <div class="form-group">
        ...
        <div class="box box-primary">
            <div class="box-body">
                <button type="submit" class="btn btn-success" name="submit"><i class="fa fa-check"></i> {{ __d('system', 'Save all Changes') }}</button>
            </div>
        </div>
